...
IDENT=$(swaymsg -t get_inputs -p | grep -oP '(?i)([^\s]+(AT_Translated_Set_2_keyboard))(?-i)')
# IDENT=$(swaymsg -t get_inputs | grep -P '"identifier":' -o | grep -P '"(?i)(.*(touchpad|synaptics))(?-i)"' -o);

echo $IDENT
swaymsg input "$IDENT" events toggle

# device=$(xinput list | grep -P '[\w].*(?i)(touchpad|synaptics)(?-i)' -o | head -n1)
#
